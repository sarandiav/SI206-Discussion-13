...
def add_employee(filename, cur, conn):
    ...
    for i in data:
        ...
        job = i["job_id"]
        pay = i["salary"]
        cur.execute("INSERT OR IGNORE INTO Employee(employee_id, first_name, last_name, job_id, hire_date, salary) VALUES(?,?,?,?,?,?)",(id, first, last, job, date, pay))
        conn.commit()
